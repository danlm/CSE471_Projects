...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        isGoal = True
        #if there is no food in the tuple, its content will be zero
        if len(state[1]) != 0:
            isGoal = False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                #if the next position is one of the food positions, 
                #remove this food information from the state's information
...
def cornersHeuristic(state, problem):
    ...
    #calculate which food is nearest to the minDistanceFood
...
def foodHeuristic(state, problem):
    ...
    for foodX in allNodesList:
      ...
      for foodY in allNodesList:
          #Chebyshev distance used - works since 1-D distance has most effect
          nodeDistanceDict[foodY] = max(abs(foodX[0]-foodY[0]), abs(foodX[1] - foodY[1]))
          #nodeDistanceDict[foodY] = abs(foodX[0]-foodY[0]) + abs(foodX[1] - foodY[1])
          #nodeDistanceDict[foodY] = pow((foodX[0]-foodX[1]),2) + pow((foodY[0] - foodY[1]), 2)
